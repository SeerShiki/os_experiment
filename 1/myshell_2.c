...
char *program_directory = "/home/hm/os_experiment/mylib";
#define MAXARGS 10
#define NULL ((void *)0)
...
void mysprintf(char *buffer, const char *format, const char *s1, const char *s2) {
    char ch;
    int i = 0, j = 0, tmp = 0;
...
void set_env(const char *name, const char *value) {
    ...
    char **new_environ = zalloc((env_len + 2) * sizeof(char *));
    if (new_environ == NULL) {
        return;
    }
    ...
    char *env_str = zalloc(strlen(name) + strlen(value) + 2);
//     if (env_str == NULL) {
//         free(new_environ);
//         return;
//     }
    mysprintf(env_str, "%s=%s", name, value);
    for (int i = 0; i < env_len; i++){
...
void runcmd(struct cmd* cmd) {
  ...
  switch (cmd->type) {
    case EXEC:
      ...
      char full_path[100];
      mysprintf(full_path, "%s/%s",program_directory, ecmd->argv[0]);
      if (is_executable(full_path)){
        syscall(SYS_execve, full_path, ecmd->argv, NULL);
        print("fail to exec ", full_path, "\n", NULL);
      }
      else{
...
void _start() {
  set_env("PATH",program_directory);
...
/*
void _start() {
  // 设置 PATH 环境变量，将当前目录加入到 PATH 中
  char* envp[] = {"PATH=/home/hm/下载/os_experiment", NULL};
  ...
  while (getcmd(buf, sizeof(buf)) >= 0) {
    ...
    if (syscall(SYS_fork) == 0) {
      ...
      if (buf[0] == '.' && buf[1] == '/'){
        struct execcmd* ecmd;
        syscall(SYS_execve, parsecmd(buf), NULL, envp);
        syscall(SYS_execve, ecmd->argv[0], ecmd->argv, NULL);
        print("fail to exec ", ecmd->argv[0], "\n", NULL);
        continue;
      }
...
//   char full_path[100];
//   mysprintf(full_path, "%s/%s",program_directory, cmd->argv[0]);
//   if (syscall(SYS_fork) == 0) {
//     syscall(SYS_execve, full_path, cmd->argv, NULL);
//     print("fail to exec ", full_path, "\n", NULL);
//   }
  return ret;
}
